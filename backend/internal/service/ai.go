...
重要规则:
1. 保持 module 的 id、type、children 结构不变
2. 只修改 styles 和 props 中的值
3. 返回的 JSON 必须是完整的 module 对象
4. 使用微信推文安全的样式（避免使用不支持的内联样式如 background-image、box-shadow 等）
5. 风格应适合微信公众号文章，不过度花哨
