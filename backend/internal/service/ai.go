...
模块(module)的 JSON 结构如下:
{
  ...
  "styles": {
    ...
    "borderLeft": "左侧边框, 如 '4px solid #3b82f6'",
...
- text: { content: "HTML文本内容", icon?: "左上角图标，如emoji" }
...
- quote: { content: "引用文字", author?: "作者或来源" }
...
const pageSystemPrompt = `你是一个微信公众号文章排版助手。你的任务是根据用户的请求，修改整页的样式设置。

页面样式(pageStyles)的 JSON 结构如下:
{
  "backgroundColor": "页面背景色, 如 '#ffffff' 或 'transparent'"
}

重要规则:
1. backgroundColor 可以是任何合法的 CSS 颜色值
2. 如果要让页面背景透明，设置为 "transparent"
3. 推荐使用柔和、适合阅读的背景色，如 '#f5f7fa', '#fefefe', '#fff9f0' 等
4. 使用微信推文安全的样式

请用以下 JSON 格式返回:
{
  "explanation": "用中文说明你做了哪些修改",
  "pageStyles": { /* 完整的更新后的 pageStyles 对象，仅包含 backgroundColor */ }
}
...
// GenerateSuggestion sends data to LLM and returns suggestion
func GenerateSuggestion(moduleJSON, prompt, mode string, dbCfg *model.AIConfig) (explanation, updatedModule, updatedPageStyles string, err error) {
	isPageMode := moduleJSON == "" || mode == "page"

	var selectedPrompt string
	var userPrompt string

	if isPageMode {
		selectedPrompt = pageSystemPrompt
	...
	if isPageMode {
		var result struct {
			...
			PageStyles  string `json:"pageStyles"`
		}
		if err := json.Unmarshal([]byte(cleaned), &result); err != nil {
			var altResult struct {
				Explanation string          `json:"explanation"`
				PageStyles  json.RawMessage `json:"pageStyles"`
			}
			...
			psBytes, _ := json.Marshal(altResult.PageStyles)
			return altResult.Explanation, "", string(psBytes), nil
		}
		return result.Explanation, "", result.PageStyles, nil
	}
